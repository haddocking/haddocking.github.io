...
the web server also summarizes the DisVis output highlighting relevant information and providing a
first overview of the interaction space between the two molecules with images autogenerated in [UCSF Chimera][link-chimera].
...
Throughout the tutorial, colored text will be used to refer to questions, 
...
Using Chimera, we can easily visualize the models and the identified cross-links, by navigating
...
If you want to use the Chimera command-lineinstead, you need to first display it:
...
We will try to assess the consistency of those restraints with DisVis and figure out which of those restraints might be false positives
...
This is also the minial required input for the web server in order to setup a run.
To run DisVis, go to
...
Click on the "**Submit**" menu to access the [input form][link-disvis-submit].
...
statistics with the highest number of violations. The DisVis web server preformats the results in a way that false positive restraints 
...
were found, here **7**. Try to change the level in the "**Volume Viewer**"to see how the addition of restraints reduces
...
and the scanning chain. We used [NACCESS](http://www.bioinf.manchester.ac.uk/naccess/) to compute the
...
recently published [article](https://dx.doi.org/10.15252/embj.201695222) (november 2016) structure of a homologue 
...
Our docking program, [HADDOCK](http://bonvinlab.org/software/haddock2.2), also available as a [web server](http://haddock.science.uu.nl/services/HADDOCK2.2), 
...
[tutorial](http://bonvinlab.org/education/HADDOCK-protein-protein-basic).
...
our [DisVis forum](http://ask.bioexcel.eu/c/disvis) hosted by the [<img width="70" src="/images/Bioexcel_logo.png">](http://bioexcel.eu) center of excellence 
