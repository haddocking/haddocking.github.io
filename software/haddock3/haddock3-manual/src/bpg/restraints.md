...
As you probably saw in the [previous step dedicated at structure preparation](./structures.md), there are many ways how to obtain structures of molecules that you want to dock.
...
If your predictions are **highly reliable** and you wish to have all of them applied during docking, define them a [**unambiguous restraints**](../intro_restraints.md#unambiguous-distance-restraints) (using the `unambig_fname` parameter).
...
* [Hydrogen bonds restraints](../intro_restraints.md#hydrogen-distance-restraints): An other type of restraints not subject to random removal (accessed using `hbond_fname` parameter).
...
If there is no direct information about the interacting residues available, one can still browse through the available literature or employ bionformatic prediction tools to gain some information about the potential complex.
...
**IMPORTANT NOTE**: Spliting your *very* ambiguous interaction restraints into multiple files can allow further de-noising (in addition to `randremoval = true`). This is performed by generating multiple restraints files, combining them in a single `.tgz` archive and finally using it from the `ambig_fname` parameter.
...
In [BonvinLab](https://www.bonvinlab.org), a number of complementary webservers have been developed to help users to reevaluate restraints. 

## [ARCTIC-3D](https://wenmr.science.uu.nl/arctic3d/)

[ARCTIC-3D](https://wenmr.science.uu.nl/arctic3d/), standing for ..., is a data mining alrogithm that search for experimental interfaces in the PDB and cluster interaction sites together. It is also able to directly generate AIRs for haddock3.
